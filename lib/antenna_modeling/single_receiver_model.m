...
tx = txsite(Name="Small cell transmitter", ...
    ...
    AntennaHeight=20, ...
    TransmitterPower=1, ...
    TransmitterFrequency=2.8e9); % 2.8 GHz
show(tx)
...
% Теперь расчет RSRP, учитывая ширину полосы
SignalWidth = 20e6; % Ширина полосы в Гц (например, 20 MHz)
RSRP = ss + 10*log10(SignalWidth);
disp("Calculated RSRP: " + RSRP + " dBm")
