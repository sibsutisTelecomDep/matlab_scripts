...
% Добавляем вычисление количества поднесущих
bw_mhz = 10; % ширина полосы в МГц (например, 10 МГц)
subcarrier_spacing = 15e3; % шаг между поднесущими в 15 кГц
N = floor((bw_mhz * 1e6) / subcarrier_spacing); % количество поднесущих
...
% Логика для расчетов с использованием библиотек (если доступны)
...
if exist('parfor', 'file')
    parfor i = 1:numComb
        ...
        RSSI_LOS = P_Tx_dBm_local - L_cable_local - PL_LOS_base;
        RSRP_LOS = RSSI_LOS - 10*log10(N); % вычисление RSRP для LOS

        PL_NLOS = PL_NLOS_base + normrnd(0, shadow_std_local, numRows, 1);
        RSSI_NLOS = P_Tx_dBm_local - L_cable_local - PL_NLOS;
        RSRP_NLOS = RSSI_NLOS - 10*log10(N); % вычисление RSRP для NLOS
        
        if ~all(isnan(PL_LOS_lib_base))
            PL_NLOS_lib = PL_NLOS_lib_base + normrnd(0, shadow_std_local, numRows, 1);
            RSSI_LOS_lib = P_Tx_dBm_local - L_cable_local - PL_LOS_lib_base;
            RSRP_LOS_lib = RSSI_LOS_lib - 10*log10(N);
            RSRP_NLOS_lib = RSSI_NLOS_lib - 10*log10(N);
        else
            RSRP_LOS_lib = NaN(numRows, 1);
            RSRP_NLOS_lib = NaN(numRows, 1);
        end
        
        diff_nlos = rsrpPhone - RSRP_NLOS;
        diff_nlos_lib = rsrpPhone - RSRP_NLOS_lib;
        ...
        tempFullTable = table(...
            ...
            diff_nlos, ...
            diff_nlos_lib, ...
            'VariableNames', {'P_Tx_dBm', 'L_cable', 'shadow_std', 'lat', 'lon', 'ci', 'distance', 'rsrpPhone', 'RSRP_LOS', 'RSRP_NLOS', 'RSRP_LOS_lib', 'RSRP_NLOS_lib', 'diff_nlos', 'diff_nlos_lib'});
    ...
    for i = 1:numComb
        ...
        RSSI_LOS = P_Tx_dBm_local - L_cable_local - PL_LOS_base;
        RSRP_LOS = RSSI_LOS - 10*log10(N);
        
        PL_NLOS = PL_NLOS_base + normrnd(0, shadow_std_local, numRows, 1);
        RSSI_NLOS = P_Tx_dBm_local - L_cable_local - PL_NLOS;
        RSRP_NLOS = RSSI_NLOS - 10*log10(N);
